...
router.post("/register", async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({
        messege: `Staff member (${role}) registerd successfully`,
        staff_id,
      });
  } catch (error) {
    res.status(500).json({
...
router.put("/:staff_id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to update staff member", details: err.message });
  }
});
...
router.delete("/:staff_id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to delete staff member", details: err.message });
  }
});
...
router.get("/", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch staff members", details: err.message });
  }
});
